...
<?php
    ...
    if(isset($_POST['action'])){
        ...
        // BNQR gateway actions - bypass CSRF (public-facing)
        if (in_array($action, ['bnqr-verify', 'bnqr-poll', 'bnqr-cancel', 'bnqr-free-slot'])) {
            $bnqrFile = __DIR__ . '/../pp-modules/pp-gateways/bangla-qr/bangla-qr.php';
            if (file_exists($bnqrFile)) {
                while (ob_get_level()) ob_end_clean();
                ob_start();
                require_once $bnqrFile;
                if ($action === 'bnqr-verify') $result = bnqr_handle_verify();
                elseif ($action === 'bnqr-poll') $result = bnqr_handle_poll();
                elseif ($action === 'bnqr-cancel') $result = bnqr_handle_cancel();
                elseif ($action === 'bnqr-free-slot') $result = bnqr_handle_free_slot();
                ob_end_clean();
        ...
        if($action == ""){
            ...
            if($action == "general-setting"){
                if($global_user_login == true){
                    ...
                    $auto_verify_type = escape_string($_POST['auto_verify_type'] ?? 'trxid');
                    ...
                    if($autoExchange == ""){
                        ...
                        $columns = ['autoExchange', 'favicon', 'logo', 'name', 'timezone', 'language', 'currency_code', 'payment_tolerance', 'auto_verify_type', 'street_address', 'city_town', 'postal_code', 'country', 'support_phone_number', 'support_email_address', 'support_website', 'whatsapp_number', 'telegram', 'facebook_messenger', 'facebook_page', 'updated_date'];
                        $values = [$autoExchange, $branding_favicon, $branding_primary_logo, $site_name, $default_timezone, $default_language, $default_currency, money_sanitize($payment_tolerance), $auto_verify_type, $street_address, $city_town, $postal_code, $country, $support_phone_number, $support_email_address, $support_website, $whatsapp_number, $telegram, $facebook_messenger, $facebook_page, getCurrentDatetime('Y-m-d H:i:s')];
    ...
    if(isset($_POST['action-v2'])){
        ...
        if($action == ""){
            ...
            if (!isset($_SESSION['csrf_token']) || !hash_equals((string)$_SESSION['csrf_token'], (string)$csrfSubmitted)) {
                echo json_encode(['status' => "false", 'title' => 'Security check failed', 'message' => 'Your session expired. Please refresh the page and try again.']);
                exit();
            }

            if($action == "phone-verify"){ pp_handle_phone_verify(); exit(); }
            if($action == "phone-cancel"){ pp_handle_phone_cancel(); exit(); }
            if($action == "phone-set"){ pp_handle_phone_set(); exit(); }
            if($action == "phone-poll"){ pp_handle_phone_poll(); exit(); }
            if($action == "phone-fallback"){ pp_handle_phone_fallback(); exit(); }
            if($action == "invoice"){
                $itemid = escape_string($_POST['itemid'] ?? '');
