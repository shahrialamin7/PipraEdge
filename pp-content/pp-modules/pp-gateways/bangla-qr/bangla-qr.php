...
function bnqr_handle_verify($data = null) {
    ...
    if (!$verified) {
        ...
        if ($uaRow && !empty($uaRow['unique_amount'])) {
            ...
            $tolerance = (float)($brand['payment_tolerance'] ?? 1);

            $findSql = 'SELECT id, trx_id, number, amount, sender_key, sender, type FROM '.$db_prefix.'sms_data 
                        WHERE sender_key = :sender_key AND type = :type AND amount BETWEEN :low AND :high AND status = :approved 
                        ORDER BY id DESC LIMIT 10';
            $findStmt = $pdo->prepare($findSql);
            $findStmt->execute([
                ':sender_key' => $provider_key, 
                ':type' => 'Merchant',
                ':low' => $unique_amount - $tolerance, 
                ':high' => $unique_amount + $tolerance, 
                ':approved' => 'approved'
            ]);
            ...
            foreach ($candidates as $sms) {
                $diff = abs((float)$sms['amount'] - $unique_amount);
...
function bnqr_generate_dynamic_qr($gateway_id, $unique_amount, $reference_label = null, $store_label = null) {
    ...
    try {
        $png = bnqr_generate_qr($dynamicPayload, 300, 2, $logoPath);
 ...
 * Uses unique amount matching first, then falls back to tolerance-based matching
 */
function bnqr_handle_poll($data = null) {
    ...
    // ── FALLBACK: Tolerance-based amount matching ──────────────────
    $tolerance = (float)($brand['payment_tolerance'] ?? 1);
    $tolerance_pct = $tolerance / 100;
    $low  = money_sub($convertedAmount, money_mul($convertedAmount, $tolerance_pct, 8), 8);
    $high = money_add($convertedAmount, money_mul($convertedAmount, $tolerance_pct, 8), 8);
        ...
        foreach ($candidates as $sms) {
            ...
            if ($claimStmt->rowCount() > 0) {
                ...
                $moreinfo = [
                    ['label' => 'Provider',      'value' => ucfirst($provider)],
                    ['label' => 'Mobile Number',  'value' => $sms['number']],
                    ['label' => 'Match Method',   'value' => 'auto-poll'],
                ];
